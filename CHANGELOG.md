...
- `archive/original/` and the root `original/`, `sample-data/`, and
  `screenshots/` folders (all original-artifact copies, none referenced
...
- README's Repository Structure still said `gaussian-elimination-solver/`
  from before the product-name rebrand; corrected to `fulcrum/`.
